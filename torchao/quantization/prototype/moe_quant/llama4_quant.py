...
class ConditionalFeedForwardAOQuantizable(nn.Module):
    ...
    def forward(
        ...
        if x.shape[0]==1: #only 1 token (can be done without graph breaks when compiled)
            ...
            w1 = self.w1[expert_indices].view(num_token_activations, self.expert_dim, self.hidden_dim)
            w2 = self.w2[expert_indices].view(num_token_activations, self.hidden_dim, self.expert_dim)
            w3 = self.w3[expert_indices].view(num_token_activations, self.expert_dim, self.hidden_dim)
            ...
            # combine outputs
            final_out = (torch.cat(outs, dim=0) * expert_weights.view(-1,1)).sum(dim=0).reshape(x.shape)
            return final_out
        else:
            ...
            num_tokens_per_expert = torch.histc(expert_indices.to("cuda"), bins=self.num_experts+1, min=-1, max=self.num_experts).to(device) #  [E+1] (added leading 0 so can be used for indexing)
            ...
            final_out = final_out.scatter_add(dim=0, index=ordered_token_indices.unsqueeze(-1).expand(num_token_activations, self.hidden_dim).to(torch.int64), src=weighted_ordered_outs)
        return final_out
...
prompt = "He is here, teh one who will tear apart the very stars in heaven."
