...
def _int4_weight_only_transform(
    ...
    if "ConditionalFeedForwardAOQuantizable" in str(type(module)):
        ...
        assert hasattr(module, "weight"), (
            f"applying int8 weight only quant requires module to have weight attribute"
...
def _int8_dynamic_activation_int8_weight_transform(
    ...
    if "ConditionalFeedForwardAOQuantizable" in str(type(module)):
        ...
        assert hasattr(module, "weight"), (
            f"applying int8 dynamic activation int8 weight quant requires module to have weight attribute"
            + "but {module} does not have one"
        )
        new_weight = _int8_dynamic_activation_int8_weight_quantize_tensor(module.weight, config)
        module.weight = torch.nn.Parameter(new_weight, requires_grad=False)
        module.extra_repr = types.MethodType(_linear_extra_repr, module)
    return module
...
def _float8_dynamic_activation_float8_weight_transform(
    ...
    if "ConditionalFeedForwardAOQuantizable" in str(type(module)):
        for weight_attr in ["w1", "w2", "w3"]:
            weight = getattr(module, weight_attr)
            quantized_weight = _float8_dynamic_activation_float8_weight_quantize_tensor(weight, config)
            new_weight = torch.nn.Parameter(quantized_weight, requires_grad=False)
            setattr(module, weight_attr, new_weight)
        return module
    else:
        assert hasattr(module, "weight"), (
            f"applying float8 dynamic activation quant requires module to have weight attribute"
            + "but {module} does not have one"
        )
        quantized_weight = _float8_dynamic_activation_float8_weight_quantize_tensor(module.weight, config)
